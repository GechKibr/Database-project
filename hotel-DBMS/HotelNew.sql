...
CREATE TABLE guest (
    ...
    FULLTEXT INDEX ft_guest_names (firstName, lastName)
) ;
...
create table payment (
    ...
    INDEX idx_payment_reservation (reservationId),
...
CREATE TABLE inventory (
    ...
    FULLTEXT INDEX ft_inventory_item (itemName, category)
) ENGINE=InnoDB;
...
CREATE TABLE services (
    ...
    FULLTEXT INDEX ft_service_desc (serviceName, description)
) ENGINE=InnoDB;
...
FOR EACH ROW
BEGIN
    -- Check if status changed to 'completed' and cost is positive
    IF NEW.status = 'completed' AND OLD.status != 'completed' AND NEW.cost > 0 THEN
        -- Update staff salary with bonus (5% of maintenance cost)
        UPDATE staff 
        SET salary = salary + (NEW.cost * 0.05)
        WHERE staffId = NEW.staffId;
        
        -- Optional: Log the bonus (recommended for auditing)
        INSERT INTO maintenance_bonus_log (staffId, maintenanceId, bonusAmount, paymentDate)
        VALUES (NEW.staffId, NEW.maintenanceId, NEW.cost * 0.05, NOW());
    END IF;
END //
...
-- Reported (2 records)
(1, 14, 'AC not cooling', 'high', '2025-03-03 09:15:00', NULL, NULL, 'reported', 'Guest reports AC not cooling room sufficiently', 0.00),
(20, 16, 'Leaky faucet', 'medium', '2025-03-10 14:30:00', NULL, NULL, 'reported', 'Bathroom faucet dripping constantly', 0.00),



-- Completed (3 records)
(19, 14, 'Stuck window', 'low', '2025-03-05 10:00:00', '2025-03-05 14:00:00', '2025-03-05 15:30:00', 'completed', 'Window track cleaned and lubricated', 15.00),
...
select*from staff_compensation_view ;
...
CALL sp_make_reservation(1, 1, '2023-12-01', '2023-12-05', 2, 0);  -- John Doe, Room 101 (Standard)
...
CALL sp_make_reservation(5, 20, '2023-12-20', '2024-01-05', 2, 0);  -- Michael Brown, Room 202 (Deluxe)
